...
async function getAllScreenshots(steamID) {
    ...
    try {
        ...
        // Try one more check for missing screenshots if we know there should be more
        if (screenshots.length < totalScreenshots && totalScreenshots > 0) {
            console.log(`Found ${screenshots.length} screenshots, but profile indicates ${totalScreenshots} total.`);
            console.log(`Trying additional methods to find the missing ${totalScreenshots - screenshots.length} screenshots...`);
            
            // One method is to try different sorting and filtering options
            // We'll try a few more combinations with varying page sizes
            const additionalViewTypes = [
                "?appid=0&sort=score", // Sort by rating/score
                "?appid=0&browsefilter=trending", // Trending filter
                "?browsefilter=myfavorites", // Favorites filter
                "?view=largeimage" // Large image view
            ];
            
            for (const viewType of additionalViewTypes) {
                const url = `${profileUrl}${viewType}`;
                try {
                    console.log(`Trying additional view: ${url}`);
                    const response = await fetchWithRetry(url, { headers });
                    const html = await response.text();
                    
                    // Process this page with the same patterns as before
                    // (Code would be similar to the page processing above)
                    // ...
                    
                    // Add delay between these additional requests
                    await new Promise(resolve => setTimeout(resolve, 1500));
                } catch (err) {
                    console.error(`Error with additional view ${url}:`, err);
                }
            }
        }
...
app.get("/screenshot/:id", async (req, res) => {
    ...
    try {
        ...
        const extractionMethods = [
            ...
            // Method 4: Any image with jpg or png extension
            () => {
                const regex = /<img[^>]+src="(https:\/\/[^"]+\.(jpg|png))[^"]*"/i;
                const match = html.match(regex);
                return match ? match[1] : null;
            }
        ];
